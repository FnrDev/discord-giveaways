...
class GiveawaysManager extends EventEmitter {
    ...
    generateMainEmbed(giveaway, lastChanceEnabled = false) {
        const embed = new Discord.MessageEmbed()
            .setAuthor({ name: giveaway.role ? '⚠ YOU NEED SPECIFIC ROLE TO JOIN' : null })
    ...
    start(channel, options) {
        return new Promise(async (resolve, reject) => {
            ...
            const giveaway = new Giveaway(this, {
                ...
                banner: typeof options.banner === 'string' ? options.banner : undefined,
                ...
                role: options.role ? options.role.id : undefined
            });
